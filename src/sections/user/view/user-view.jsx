...
export default function UserPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Add document data and image URLs to Firestore
      await addDoc(collection(db, 'assets'), {
        ...
        addedBy,
        status,
        imageUrls: imageUrls, // Assuming 'imageUrls' is an array of image download URLs
      });
  ...
  const ImageDropzone = () => {
    ...
    return (
      <div {...getRootProps()} style={{ border: '2px dashed #eeeeee', padding: '20px', marginTop: '20px' }}>
        <input {...getInputProps()} />
        <p>Drag 'n' drop some images here, or click to select images</p>
        <div>
          {selectedImages.map((file, index) => (
            <div key={index} style={{ display: 'inline-block', marginRight: '10px' }}>
              <img src={file.preview} alt={`preview-${index}`} style={{ width: '100px', height: '100px', objectFit: 'cover' }} />
            </div>
          ))}
        </div>
      </div>
